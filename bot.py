...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None):
        ...
        log.info(' MusicBot (버전 {}) '.format(BOTVERSION).center(50, '='))

        if not self.autoplaylist:
            log.warning("자동 재생 목록이 비어 있습니다.")
            self.config.auto_playlist = False
        else:
            log.info("{} 개의 항목이있는 자동 재생 목록로드 됨".format(len(self.autoplaylist)))

        if self.blacklist:
            log.debug("{} 개 항목이있는 블랙리스트로드 됨".format(len(self.blacklist)))
    ...
    def dev_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if orig_msg.author.id in self.config.dev_ids:
                ...
                raise exceptions.PermissionsError("dev 사용자 만이 명령을 사용할 수 있습니다.", expire_in=30)
    ...
    async def _check_ignore_non_voice(self, msg):
        ...
        if not vc or vc == msg.author.voice_channel:
            ...
            raise exceptions.PermissionsError(
                "음성 채널 (% s)에 없을 때이 명령을 사용할 수 없습니다." % vc.name, expire_in=30)
            ...
            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        '봇은 음성 채널에 없습니다.  '
                        '% 소환을 사용하여 음성 채널에 소환하십시오.' % self.config.command_prefix)
        ...
        if not player.playlist.entries and not player.current_entry and self.config.auto_playlist:
            ...
            while self.autoplaylist_session:
                ...
                if info.get('entries', None):  # or .get('_type', '') == 'playlist'
                    log.debug("재생 목록이 발견되었지만 건너 뛰는 현재 지원되지 않습니다.")
...
#######################################################################################################################
    ...
    async def on_ready(self):
        ...
        if owner and self.servers:
            ...
            log.warning("소유자 알 수 없음, 봇이 어떤 서버에도 없습니다.")
            if self.user.bot:
                log.warning(
        ...
        # 어쩌면 ownerid를 비워두고 소유자가 사용할 임의의 명령을 생성하는 옵션 일 수도 있습니다.
        # wait_for_message는 꽤 neato입니다.
    ...
    async def cmd_help(self, command=None):
        ...
        도움말 메시지를 인쇄합니다.
        명령이 지정되면 해당 명령에 대한 도움말 메시지를 인쇄합니다.
        그렇지 않으면 사용 가능한 명령을 나열합니다.
        """
        ...
        if command:
            ...
            if cmd and not hasattr(cmd, 'dev_cmd'):
                ...
                return Response("그러한 명령이 없습니다.", delete_after=10)
            ...
            helpmsg += "각 명령에 대한 자세한 정보는`{} help x`를 사용할 수도 있습니다.".format(self.config.command_prefix)
    ...
    async def cmd_blacklist(self, message, user_mentions, option, something):
        ...
        블랙리스트에 사용자를 추가하거나 제거하십시오.
        블랙리스트에 올라있는 사용자는 봇 명령 사용을 금합니다.
        """
        ...
        if option not in ['+', '-', 'add', 'remove']:
            raise exceptions.CommandError(
                '잘못된 옵션 "% s"이 (가) 지정되었습니다. +, -, 추가 또는 제거를 사용합니다.' % option, expire_in=20
            )

        for user in user_mentions.copy():
            if user.id == self.config.owner_id:
                print("[Commands : Blacklist] 소유자를 블랙리스트에 올릴 수 없습니다.")
    ...
    async def cmd_save(self, player):
        ...
        if player.current_entry and not isinstance(player.current_entry, StreamPlaylistEntry):
            ...
            if url not in self.autoplaylist:
                ...
                raise exceptions.CommandError('이 노래는 이미 자동 재생 목록에 있습니다.')
        else:
            raise exceptions.CommandError('유효한 노래가 없습니다.')
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        봇에게 서버에 가입하도록 요청합니다. 참고 : 봇 계정은 초대 링크를 사용할 수 없습니다.
        """
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
            {command_prefix}play 링크
            {command_prefix}play 이름 및 검색

        노래를 재생 목록에 추가합니다.   링크가 제공되지 않으면 첫 번째
          YouTube 검색 결과가 대기열에 추가됩니다.
        """
        ...
        async with self.aiolocks[_func_() + ':' + author.id]:
            if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
                raise exceptions.PermissionsError(
                    "대기열에있는 노래 한도 (% s)에 도달했습니다." % permissions.max_songs, expire_in=30
                )
            ...
            if not info:
                raise exceptions.CommandError(
                    "해당 동영상을 재생할 수 없습니다. {} 스트림 명령을 사용해보십시오.".format(self.config.command_prefix),
                    expire_in=30
                )

            # 사용자로부터 검색 처리를 추상화합니다.
            if info.get('url', '').startswith('ytsearch'):
                ...
                if not info:
                    raise exceptions.CommandError(
                        "검색 문자열에서 정보를 추출하는 중 오류가 발생했습니다. youtubedl은 데이터를 반환하지 않았습니다."
                        "이런 일이 계속되면 봇을 다시 시작해야 할 수도 있습니다.", expire_in=30
                    )

                if not all(info.get('entries', [])):
                    # empty list, no data
                    log.debug("빈 목록, 데이터 없음")
                    return

                # TODO : 'webpage_url'인 'ytsearch : ...'또는 추출기 유형 처리
            ...
            if 'entries' in info:
                ...
                if permissions.max_songs and player.playlist.count_for_user(author) + num_songs > permissions.max_songs:
                    raise exceptions.PermissionsError(
                        "재생 목록 항목 + 이미 대기중인 노래가 한도에 도달했습니다. (% s + % s> % s)" % (
                ...
                if info['extractor'].lower() in ['youtube:playlist', 'soundcloud:set', 'bandcamp:album']:
                    try:
                        ...
                        log.error("오류 대기열 재생 목록", exc_info=True)
                        raise exceptions.CommandError("오류 대기열 재생 목록 : \ n % s" % e, expire_in=30)
                ...
                procmesg = await self.safe_send_message(
                    channel,
                    '{} 곡 {}에 대한 재생 목록 정보 수집 중'.format(
                        num_songs,
                        ', 예상 소요 시간 : {} 초'.format(fixg(
                ...
                if permissions.max_song_length:
                    ...
                    if drop_count:
                        print("노래 % s 곡 삭제됨" % drop_count)

                log.info("{: .2f} s / 노래, {: + .2g} / 예상 노래 ({} s)에서 {} 초 만에 {} 곡을 처리했습니다.".format(
                    listlen,
                    fixg(ttime),
                ...
                if not listlen - drop_count:
                    raise exceptions.CommandError(
                        "노래를 추가하지 않았습니다. 모든 노래가 최대 재생 시간 (% ss) 이상이었습니다." % permissions.max_song_length,
                        expire_in=30
                    )

                reply_text = "** Enqueued ** % s ** 재생할 노래. 대기열의 위치 : % s"
                btext = str(listlen - drop_count)

            else:
                if permissions.max_song_length and info.get('duration', 0) > permissions.max_song_length:
                    raise exceptions.PermissionsError(
                        "노래 길이가 한도를 초과했습니다 (% s> % s)." % (info['duration'], permissions.max_song_length),
                        expire_in=30
                    )
                ...
                try:
                    ...
                    if e.use_url == song_url:
                        log.warning("잘못된 입력 유형이 결정되었지만 제안 된 URL은 같습니다. 도움.")
                    log.debug("Using \"%s\" instead" % e.use_url)

                    return await self.cmd_play(player, channel, author, permissions, leftover_args, e.use_url)

                reply_text = "대기열에 넣기 ** % s **를 재생합니다. 대기열의 위치 : % s"
                btext = entry.title
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        """
        비밀 처리기가 비동기 마법을 사용하여 재생 목록 대기열을 '차단'하지 않습니다.
        """
        ...
        if not info:
            raise exceptions.CommandError("해당 재생 목록은 재생할 수 없습니다.")
    ...
    async def cmd_stream(self, player, channel, author, permissions, song_url):
        ...
        미디어 스트림을 큐에 넣습니다.
        이것은 Twitch 나 shoutcast와 같은 실제 스트림을 의미하거나 단순히 스트리밍을 의미 할 수 있습니다.
        미디어를 미리 다운로드하지 않아도됩니다.   참고 : FFmpeg는 악명이 높습니다.
         스트림, 특히 열악한 연결.   당신은 경고를 받았습니다.
        """
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "이제 {action} : ** {title} ** ** {author} ** {progress} 님이 추가했습니다. \ n \ N {WHITE RIGHT POINTING BACKHAND INDEX} <{url}>".format(
                    action=action_text,
    ...
    async def cmd_list(self, channel, server, author, voice_channel):
        """
        Usage:
            {command_prefix}list

        소환사의 음성 채널로 봇을 호출하십시오.
        """

        if not author.voice_channel:
            raise exceptions.CommandError('음성 채널에 있지 않습니다.')
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel):
        ...
        충분한 득표가 이루어지면 봇 소유자가 현재 노래를 건너 뜁니다.
        """

        if player.is_stopped:
            raise exceptions.CommandError("건너 뛸 수 없습니다! 플레이어가 재생되지 않습니다!", expire_in=20)

        if not player.current_entry:
            if player.playlist.peek():
                if player.playlist.peek()._is_downloading:
                    return Response("다음 노래 (% s)가 다운로드 중입니다. 잠시 기다려주십시오." % player.playlist.peek().title)
                elif player.playlist.peek().is_downloaded:
                    print("다음 노래가 곧 재생됩니다. 기다려주십시오.")
                else:
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        재생 볼륨을 설정합니다. 허용되는 값은 1에서 100 사이입니다.
        볼륨 앞에 + 또는 -을 붙이면 현재 볼륨을 기준으로 볼륨이 변경됩니다.
        """

        if not new_volume:
            return Response('현재 볼륨 :`% s %%`' % int(player.volume * 100), reply=True, delete_after=20)
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        봇이 채팅에 게시 한 [범위] 메시지를 제거합니다. 기본값 : 50, 최대 : 1000
        """
    ...
    async def cmd_listids(self, server, author, leftover_args, cat='all'):
        ...
        다양한 항목에 대한 ID를 나열합니다.   카테고리는 다음과 같습니다.:
            모두, 사용자, 역할, 채널
        """
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        봇의 아바타를 변경합니다.
        파일을 첨부하고 URL 매개 변수를 비워두면 작동합니다.
        """
    ...
    async def on_server_remove(self, server: discord.Server):
        log.info("Bot이 서버에서 삭제되었습니다 : {}".format(server.name))
